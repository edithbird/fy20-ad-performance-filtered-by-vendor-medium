...
```{r}
data1 <- FMVM %>% 
  filter(Product != "UNI" & Code_Medium == "DISP" & Code_Vendor == "PIN" & Term %in% desiredRecruiting & Impressions > 0 ) %>% 
  group_by(Term, Product, Audience,Date) %>% 
  summarise(
    Impressions = sum(Impressions),
    Cost = sum(Cost),
...
```{r}
data1 <- FMVM %>% 
  filter(Product != "UNI" &  Code_Vendor == "SC" & Term %in% desiredRecruiting & Impressions > 0 ) %>% 
  group_by(Term, Product, Audience,Date) %>% 
  summarise(
    Impressions = sum(Impressions),
    Cost = sum(Cost),
...
```{r}
data1 <- FMVM %>% 
  filter(Product != "UNI" &  Code_Vendor == "LI" & Code_Medium == "TEXT" & Term %in% desiredRecruiting & Sends > 0 ) %>% 
  group_by(Term, Product, Audience,Date) %>% 
  summarise(
    Sends = sum(Sends),
    Cost = sum(Cost),
...
```{r}
data1 <- FMVM %>% 
  filter(Product != "UNI" &  Code_Vendor == "GM" & Term %in% desiredRecruiting & Impressions > 0 ) %>% 
  group_by(Term, Product, Audience,Date) %>% 
  summarise(
    Impressions = sum(Impressions),
    Cost = sum(Cost),
...
```{r}
data1 <- FMVM %>% 
  filter(Product != "UNI" &  Code_Vendor == "TW" & Term %in% desiredRecruiting & Impressions > 0 ) %>% 
  group_by(Term, Product, Audience,Date) %>% 
  summarise(
    Impressions = sum(Impressions),
    Cost = sum(Cost),
...
```{r}
data1 <- FMVM %>% 
  filter(Code_Medium == "VID" & Code_Vendor == "FB" & Term %in% desiredRecruiting & Views > 0 ) %>% 
  group_by(Term, Product, Audience,Date) %>% 
  summarise(
    Impressions = sum(Impressions),
    Cost = sum(Cost),
...
```{r}
data1 <- FMVM %>% 
  filter(Code_Medium == "VID" & Code_Vendor == "IG" & Term %in% desiredRecruiting & Views > 0) %>% 
  group_by(Term, Product, Audience,Date) %>% 
  summarise(
    Impressions = sum(Impressions),
    Cost = sum(Cost),
...
```{r}
data1 <- FMVM %>% 
  filter(Code_Medium == "VID" & Code_Vendor == "YT" & Term %in% desiredRecruiting & Impressions >0 ) %>% 
  group_by(Term, Product, Audience,Date) %>% 
  summarise(
    Impressions = sum(Impressions),
    Cost = sum(Cost),
 ...
 reactable(data1, searchable = TRUE, filterable = TRUE, groupBy = c("Term","Product", "Audience"), columns = list(
  ...
  Impressions = colDef(aggregate = "sum", format = colFormat(separators = TRUE),filterable = FALSE),
  UPV = colDef(aggregate = "sum",filterable = FALSE),
  CTR = colDef(aggregate = "mean", format = colFormat(percent = TRUE, digits = 2),filterable = FALSE),
  ViewRate = colDef(aggregate = "mean", format = colFormat(percent = TRUE, digits = 1),filterable = FALSE),
  VTR = colDef(aggregate = "mean", format = colFormat(percent = TRUE, digits = 1),filterable = FALSE),
  BR = colDef(aggregate = "mean",format = colFormat(percent = TRUE, digits = 0),filterable = FALSE)))
```
